<?php
$page_title = 'Edit Room - The Royal';
...
if (!$room) {
    header('Location: /the-royal/admin/rooms.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($room_number) || empty($type) || empty($description) || empty($image_url)) {
        ...
        if ($stmt->fetch()) {
            ...
            $stmt = $pdo->prepare("
                UPDATE rooms 
                SET room_number = ?, floor_number = ?, price_per_night = ?, type = ?, 
            ...
            if ($stmt->execute([$room_number, $floor_number, $price_per_night, $type, $description, $image_url, $is_available, $room_id])) {
                header('Location: /the-royal/admin/rooms.php?success=updated');
                exit();
            } else {
                ...
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="floor_number" name="floor_number" class="form-input"
                            min="1" value="<?php echo htmlspecialchars($_POST['floor_number'] ?? ''); ?>" required>
                    </div>
                    ...
                    <div class="form-group">
                        <label style="display: flex; align-items: center; cursor: pointer;">
                            <input type="checkbox" name="is_available" class="form-checkbox"
                                <?php echo ($_POST['is_available'] ?? 0) ? 'checked' : ''; ?>>
